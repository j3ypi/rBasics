...
### Eckige Klammern
Wenn man auf Elemente innerhalb eines Vektor zugreifen möchte, kann man dies mit eckigen Klammern. Das dritte Element des Vektors `vec` 
```{r}
vec <- c(1, 2, 3, 4)
```
...
Alternativ kann man auch zwei Vekoren mit `cbind()` (column bind) Spaltenweise aneinander hängen.
```{r}
cbind(1:4, 
      5:8)
```
...
Weitergehend kann man direkt auf Element der Matrix im zweiten Listenelement zugreifen. Möchte man beispielsweise den Wert in der dritten Zeile und zweiten Spalte der Matrix ausgegeben haben, schreibt man
```{r}
liste[[2]][3, 2]
```
...
Funktionen aus dem `tidyverse` stellen dabei eine Ausnahme dar. An dieser Stelle wird es leider sehr schnell komplex und kryptisch. Es sei nur erwähnt, damit bei Versuchen mit `tidyverse` Funktionen eigene Funktionen zu schreiben, kein Frust aufkommt. Haben wir zum Beispiel die einfache Aufgabe den Mittelwert und die Standardabweichung einer Variable in einem Datensatz zu berechnen, müssen wir so genannte quosures (`enquo()`) und doppelte Bang (`!!`) Operatoren benutzen, um die Aufgabe in Form einer Funktion zu automatisieren. 
```{r}
...
## Mappen 
Der Kern von funktionalem Programmieren ist Automatisierung. Erinnern wir uns zurück an die Inferenzstatistik. Genauer gesagt an das Berechnen der Informationskriterien der Modelle der hierarchischen Regression mithilfe von `glance()`. Wir haben dort drei mal den Befehl ausführen müssen. Für jedes der drei Modelle einmal. Aber was, wenn wir 10 Prädiktoren sukzessive hinzufügen wollen würden? 10 mal Copy & Paste? Eine fehleranfällige und mühsame Vorgehensweise. Mit `map()` aus dem `purrr` Package (enhalten im `tidyverse`) kann man mit 3 Zeilen Code das selbe erreichen. Zuerst speichern wir dafür die drei Modelle in einer Liste.
```{r}
...
## Nesten
Wirklich mächtig wird `map()` allerdings erst in Kombination mit `nest()`. Zu Beginn ist es durchaus abstrakt und gewöhnungsbedürftig. Aber durch das Nesten kann man gruppenweise mappen. Nehmen wir uns das Beispiel unseres `repeated` Datensatzes. Es gibt 6 verschiedene Gruppen - A bis F. Was, wenn wir nun innerhalb jeder dieser Gruppen einen F-Test zum Vergleich der Varianzen von `iq` und `kreativitaet` machen möchten? Normaler Weise müsste man sechs mal filtern und anschließend die Funktion F-Test aufrufen. Mit `nest()` benötigt man bloss 4 Zeilen Code.
```{r}
repeated %>% 
  nest(-gruppe) %>% 
...
Angenommen wir machen eine Online Umfrage mit einem kleinem Mathetest bestehend aus drei Aufgaben. Die richtigen Antworten sind 3, 2 und 4. Es nehmen vier Personen Teil. Unser Datensatz `survey` würde in etwas so aussehen
```{r, echo=FALSE}
survey <- tibble(
  I1 = c(3, 2, 3, 1),
  I2 = c(2, 2, 2, 1),
...
die richtigen Antworten 3, Apfel und 4 sind
```{r}
